...
void read_entire_file(const char *path, char **data, int *len) {
    FILE *input = fopen(path, "rb");
    if (!input) {fprintf(stderr, "Failed to open file %s\n", path); return;}
...
int main(int argc, char const *argv[]) {
    char *bin_data;
    int bin_data_len;
    read_entire_file("precompiled.bin", &bin_data, &bin_data_len);

    ShadCompilation sc;
    shad_compilation_deserialize(bin_data, bin_data_len, &sc);
